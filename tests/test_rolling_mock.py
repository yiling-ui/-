...
async def test_below_min_notional_skipped() -> None:
    """Tiny unrealised PnL should produce a < min_notional_usdt leg
    and be skipped without an order."""
    ctl, adapter, account = _build_controller(
        quote=1.0001,   # +0.001 = 0.02R, below threshold anyway
        cfg=RollingConfig(
            enabled=True, trigger_r_levels=(0.001,),    # absurdly low
...
async def test_total_notional_capped_by_leverage() -> None:
    ...
    ctl, adapter, account = _build_controller(
        ...
        cfg=RollingConfig(
            ...
            unrealized_pnl_ratio=10.0,    # absurdly high -> will bump cap
            leg_stop_pct=0.025,
        ),
    )
    pos = _open_long_position(size=4_000.0)    # existing notional = 4400@1.10
    # equity * leverage = 10_000 * 5 = 50_000. Room = 50_000 - 4400 = 45_600.
    decision = await ctl.maybe_roll(
        position=pos, account=account,
        top5_depth_usdt=400_000, realized_vol_pct=0.05,
        now_ms=1_000_000,
    ...
    assert decision.fired, decision.reason
    # Without the clamp, ratio=10.0 * unrealised_pnl ($400) / 0.0275 ≈ 145k size
    # = 160k notional -- way past the cap.
    assert decision.new_leg_notional <= 45_600.0 + 1e-3
